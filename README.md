...
| FlowLog/datatoad middle layer with DD-overlapped scheduling | Could support a long-term relational planning layer with DD execution, WCOJ-style joins, and an exact logical schedule mapped onto overlapped DD physical execution. | Requires a substantial new planner, index story, recursive-control model, timestamp/frontier design, egglog-specific adapter, and equality/rebuild invalidation model. Native actions, rebuild, and custom schedulers may still force barriers. | [Option 3](findings/options/option-3-flowlog-datatoad-middle-layer.md) |
