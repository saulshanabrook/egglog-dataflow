...
- Eli's backend draft sharpens what "seminaive deltas" means for egglog: arbitrary user schedules break the ordinary global recent/stable/new criterion, so rows carry logical timestamps and each rule tracks the last timestamp it ran at. An exact DD-backed rule layer must preserve per-rule timestamp windows, not only global relation deltas. Eli's later DD-overlap clarification suggests physical scheduling may be more flexible than egglog's current stop/start loop, but only if DD frontiers keep the logical rule windows observationally exact (`repos/scaling-equality-saturation/egglog-new-backend.md`, `messages/eli-dd-overlapped-scheduling.md`, `findings/source-notes/scaling-equality-saturation.md`).
