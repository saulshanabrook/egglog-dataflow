...
- Egglog adds a second timestamp constraint on top of Timely/DD cost: arbitrary schedules require per-rule logical freshness windows. Eli's draft says each rule records the last timestamp it ran at and queries rows inserted since that time; a DD design must preserve this rule-local seminaive behavior while still choosing a Timely timestamp granularity that compacts well (`repos/scaling-equality-saturation/egglog-new-backend.md`, `findings/source-notes/scaling-equality-saturation.md`).
