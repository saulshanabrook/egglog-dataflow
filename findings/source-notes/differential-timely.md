...
- Egglog adds a second timestamp constraint on top of Timely/DD cost: arbitrary schedules require per-rule logical freshness windows. Eli's draft says each rule records the last timestamp it ran at and queries rows inserted since that time; an exact DD design must preserve this rule-local seminaive behavior while still choosing a Timely timestamp granularity that compacts well. A relaxed scheduling design can only loosen this requirement inside an explicitly scoped mode (`repos/scaling-equality-saturation/egglog-new-backend.md`, `findings/source-notes/scaling-equality-saturation.md`).
...
- DD/Timely may generalize egglog's scheduling machinery, but the immediate verification target is stricter: reproduce egglog's per-rule timestamp freshness under arbitrary ruleset schedules before claiming schedule compatibility.
