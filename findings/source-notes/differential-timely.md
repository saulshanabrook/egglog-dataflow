...
- The first local Option 3 prototype (`code/option3-overlap`) provides positive
  but narrow evidence for that scheduling claim. On the corrected scheduled
  reachability witness, `dd-overlap` issued future logical tasks early, matched
  the oracle, and reported zero early visibility violations. Scaling remained
  mixed, and synthetic native barriers moved the run back toward stop/start
  behavior (`findings/option-3-experiments.md`).
...
- Extend the existing overlapped DD schedule prototype with richer progress and
