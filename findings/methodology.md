# Research Methodology And Source Inventory

This file preserves the detailed research framing that used to live in the
top-level README. The top-level README is now a short collaborator handoff page;
this file keeps the methodology, source inventory, reading order, and design
risks for future agents or humans who want the full context.
...
## Working Conclusions From Source Inventory

These conclusions were derived before the later synthesis and option-viability
passes. For the current consolidated conclusion, read `synthesis.md`.
...
| `../repos/scaling-equality-saturation` | `ezrosent/80190c70245632388f536fa259ec54b8`, gist submodule, `f5956bb` | Eli Rosenthal's June 2025 backend design draft. Important topics: arbitrary schedules, per-rule seminaive logical timestamps, timestamp-ordered hash tables, two-phase query/merge execution, constructor/function split, rebuild, BYODS-style table interface, Free Join, dynamic variable ordering, benchmark caveats, and Timely/Differential as future work. |
...
The local file `../repos/egglog-python/docs/explanation/2026_02_containers.md`
is the current source to read before designing around containers or higher-level
functions.
...
7. Read `../code/dd-pr-525-eqsat.rs` and identify which equality-maintenance
...
11. Read `../repos/ascent/README.MD` and `../repos/ascent/BYODS.MD` before
    deciding whether custom relation providers are a better comparison point
    than DD.
...
- The executable notebook or branch behind
  `../repos/egglog-python/docs/explanation/2026_02_containers.md`, if
  reproducing the polynomial case study becomes necessary.
