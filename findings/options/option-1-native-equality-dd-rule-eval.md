...
## Viability
- Medium. This is the smallest credible split because egglog's frontend is already close to the needed boundary: `CoreRule` is a conjunctive-query body plus SSA-like actions, and the current backend builder separately lowers body atoms to table/primitive queries and heads to `set`/`delete`/`subsume`/`union` actions (`findings/source-notes/egglog-core-proof.md`, `repos/egglog/src/core.rs`, `repos/egglog/src/lib.rs`). DD/FlowLog fits the rule-matching, seminaive delta, arrangement, and recursive fixed-point side. The viability is not High because equality changes are not ordinary relation inserts: native rebuild has value-level table rewriting, incremental/full rebuild choices, same-id dirty container refresh, and merge hooks that must be reflected back into the dataflow input without semantic misses.

## General Approach
- Keep egglog's union-find, congruence/rebuild, containers, merge functions, primitive/action VM, proof/extraction ownership, and scheduler-visible run loop native. Add a DD/FlowLog-like rule-evaluation service beside the native database. At each saturation epoch, egglog exports canonical e-node/function relation deltas and rebuild invalidation deltas to the dataflow service. The service maintains arranged relation indexes and evaluates rule bodies to produce a stream/table of matches keyed by rule id plus bound variables. Egglog then drains those matches and runs existing native actions through the current action path, including `set`, `union`, `delete`, `subsume`, primitive calls, and failed lookup behavior. After actions, native rebuild runs and emits the next batch of canonical-id/table invalidations back to dataflow.
...
- Delta stream from egglog to dataflow: inserts, deletes/subsumes, canonical-id rewrites from rebuild, class-id merge deltas old->new, and explicit invalidation/refresh events for same-id dirty containers or other semantic changes without tuple identity changes.
...
## Recommendation
- Continue, but only as a hybrid prototype. This option is worth pursuing because it preserves the hardest egglog-specific machinery natively while testing DD/FlowLog where the fit is strongest: maintained rule indexes and incremental body matching. The first milestone should not attempt relational equality maintenance; it should prove the data-exchange contract by matching native results on rebuild/container/merge regressions and quantifying class-id rewrite churn. If that contract is too large or too expensive, defer broader integration.
